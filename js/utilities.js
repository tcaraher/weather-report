...
window.weatherReport.utilities.getValue = (cityQuery, queryResult, dataUnitRequest) => {
  ...
  if (localStorage.getItem(weatherReport.constants.storageSpeedKey) === null || localStorage.getItem(weatherReport.constants.storageTemperatureKey) === null) {
    return queryResult + ' ' + suppliedDataUnit
  }
  else if (
    localStorage.getItem(weatherReport.constants.storageTemperatureKey).includes('fahrenheit') &&
