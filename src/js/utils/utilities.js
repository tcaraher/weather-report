...
export const getValue = (cityQuery, queryResult, dataUnitTypeRequested) => {
  ...
  if (localStorage.getItem(weatherReport.settingsObj.speedUnits.localStorageKey) === null || localStorage.getItem(weatherReport.settingsObj.tempUnits.localStorageKey) === null) {
    return queryResult + ' ' + fullDataQuery;
  }
  // if the setting is set to deg F
  else if (localStorage.getItem(weatherReport.settingsObj.tempUnits.localStorageKey).includes('fahrenheit') && dataUnitTypeRequested.includes('temp')) {
    return Math.round(queryResult * 1.8 + 32) + ' ' + '°F';
  }
  // if the setting is set to mph
  else if (localStorage.getItem(weatherReport.settingsObj.speedUnits.localStorageKey).includes('mph') && dataUnitTypeRequested.includes('speed')) {
